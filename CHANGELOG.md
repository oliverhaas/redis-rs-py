...
- `benchmarks/run_all.py` orchestrator: spins up Valkey via testcontainers, runs the bench tests via `pytest --codspeed --codspeed-mode=walltime`, renders side-by-side `benchmarks/RESULTS.md` from the per-session JSON dropped under `benchmarks/results/`.
- `bench.yml` GitHub Action: smoke benchmark on every PR (gate), full nightly benchmark (artifact-uploaded).
- `bench` dependency group (`pytest-codspeed`, `redis[hiredis]`, `valkey-glide`) — opt-in via `uv sync --group bench`.
...
- Sync `PubSub` and async `PubSub` (Plan 14): full redis-py-compatible pub/sub API — `subscribe`/`unsubscribe`, `psubscribe`/`punsubscribe`, `ssubscribe`/`sunsubscribe`, `get_message(timeout=)`, `aget_message(timeout=)`, `listen()` async iterator, `run_in_thread(sleep_time, exception_handler, daemon)` → `PubSubWorkerThread`, `close()`/`aclose()`, `ping()`. Auto-reconnect with subscription replay via a Tokio supervisor task using `PubSub::split()` + `tokio::select!`. Dedicated subscriber connection per `pubsub()` call. `PubSubWorkerThread` re-exported at package level; `PubSub` re-exported from `redis_rs_py.asyncio`.
