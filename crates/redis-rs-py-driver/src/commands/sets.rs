...
use crate::driver::{RedisRsDriver, py_bool, py_int, py_opt_bytes, py_set_of_bytes};
...
// =========================================================================
// #[pymethods] impl block — all set commands
// =========================================================================
...
impl RedisRsDriver {
    ...
    fn sadd(&self, py: Python<'_>, key: &str, members: Vec<Vec<u8>>) -> PyResult<Py<PyAny>> {
        ...
        let r: redis::RedisResult<i64> =
            sync_op!(py, self, conn, async { conn.sadd(key, &members).await });
    ...
    fn srem(&self, py: Python<'_>, key: &str, members: Vec<Vec<u8>>) -> PyResult<Py<PyAny>> {
        ...
        let r: redis::RedisResult<i64> =
            sync_op!(py, self, conn, async { conn.srem(key, &members).await });
        py_int(py, r.map_err(to_py_err)?)
    }
    ...
    fn smembers(&self, py: Python<'_>, key: &str) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<Vec<Vec<u8>>> =
            sync_op!(py, self, conn, async { conn.smembers(key).await });
    ...
    fn scard(&self, py: Python<'_>, key: &str) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<i64> = sync_op!(py, self, conn, async { conn.scard(key).await });
        py_int(py, r.map_err(to_py_err)?)
    }
    ...
    fn sismember(&self, py: Python<'_>, key: &str, member: &[u8]) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<bool> =
            sync_op!(py, self, conn, async { conn.sismember(key, member).await });
    ...
    #[pyo3(signature = (key, *members))]
    fn asmismember(&self, py: Python<'_>, key: &str, members: Vec<Vec<u8>>) -> PyResult<Py<PyAny>> {
        let key = key.to_string();
        async_op!(self, py, conn, async {
            if members.is_empty() {
        ...
        match count {
            None => {
                let r: redis::RedisResult<Option<Vec<u8>>> =
                    sync_op!(py, self, conn, async { conn.spop_one(key).await });
                Ok(py_opt_bytes(py, r.map_err(to_py_err)?))
            }
            Some(n) => {
                let r: redis::RedisResult<Vec<Vec<u8>>> =
                    sync_op!(py, self, conn, async { conn.spop_count(key, n).await });
                py_set_of_bytes(py, r.map_err(to_py_err)?)
            }
        }
    }
    ...
    fn srandmember(&self, py: Python<'_>, key: &str, count: Option<i64>) -> PyResult<Py<PyAny>> {
        match count {
            None => {
                let r: redis::RedisResult<Option<Vec<u8>>> =
                    sync_op!(py, self, conn, async { conn.srandmember_one(key).await });
                Ok(py_opt_bytes(py, r.map_err(to_py_err)?))
            }
            Some(n) => {
    ...
    fn sinter(&self, py: Python<'_>, keys: Vec<String>) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<Vec<Vec<u8>>> =
            sync_op!(py, self, conn, async { conn.sinter(&keys).await });
        py_set_of_bytes(py, r.map_err(to_py_err)?)
    }
    ...
    fn sunion(&self, py: Python<'_>, keys: Vec<String>) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<Vec<Vec<u8>>> =
            sync_op!(py, self, conn, async { conn.sunion(&keys).await });
        py_set_of_bytes(py, r.map_err(to_py_err)?)
    }
    ...
    fn sdiff(&self, py: Python<'_>, keys: Vec<String>) -> PyResult<Py<PyAny>> {
        let r: redis::RedisResult<Vec<Vec<u8>>> =
            sync_op!(py, self, conn, async { conn.sdiff(&keys).await });
        py_set_of_bytes(py, r.map_err(to_py_err)?)
    }
