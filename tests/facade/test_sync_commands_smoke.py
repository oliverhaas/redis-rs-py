...
def test_set_nx(r):
    assert r.set("k", "a", nx=True) is True
    assert r.set("k", "b", nx=True) is None or r.set("k", "b", nx=True) is False


def test_set_xx(r):
    assert r.set("missing", "v", xx=True) is None or r.set("missing", "v", xx=True) is False
    r.set("k", "a")
    r.set("k", "b", xx=True)
    assert r.get("k") == b"b"
...
def test_srem(r):
    r.sadd("s", "a", "b")
    assert r.srem("s", "a") == 1
    assert r.sismember("s", "a") is False
...
def test_smove(r):
    r.sadd("src", "a", "b")
    r.smove("src", "dst", "a")
    assert r.sismember("src", "a") is False
    assert r.sismember("dst", "a") is True
...
def test_zincrby(r):
    r.zadd("z", {"a": 1.0})
    val = r.zincrby("z", 2.5, "a")
    assert abs(val - 3.5) < 1e-9


def test_zscore(r):
    r.zadd("z", {"a": 1.5})
    assert abs(r.zscore("z", "a") - 1.5) < 1e-9


def test_zrank_zrevrank(r):
    r.zadd("z", {"a": 1.0, "b": 2.0, "c": 3.0})
    assert r.zrank("z", "a") == 0
    assert r.zrevrank("z", "a") == 2
...
def test_xadd_xlen_xrange(r):
    entry_id = r.xadd("stream", {"field": "value"})
...
def test_script_load_evalsha(r):
    ...
    assert isinstance(sha, bytes) and len(sha) == 40
    result = r.evalsha(sha.decode(), 0)
